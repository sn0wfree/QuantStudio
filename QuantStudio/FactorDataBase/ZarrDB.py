...
class _FactorTable(FactorTable):
    ...
    def readFactorData(self, ifactor_name, ids, dts, args={}):
        with self._FactorDB._DataLock:
            ZTable = zarr.open(self._FactorDB.MainDir + os.sep + self.Name, mode="r")
            if ifactor_name not in ZTable: raise __QS_Error__(
            ...
            Rslt = pd.DataFrame(ZFactor["Data"].get_orthogonal_selection((DTIndices, IDIndices)),
                                index=DateTimes[DTIndices], columns=IDs[IDIndices])
        if ids is not None:
            if Rslt.shape[1] > 0:
                Rslt = Rslt.loc[:, ids]
            else:
...
class ZarrDB(WritableFactorDB):
    ...
    def __init__(self, sys_args={}, config_file=None, **kwargs):
        ...
        super().__init__(sys_args=sys_args, config_file=(
            __QS_ConfigPath__ + os.sep + "ZarrDBConfig.json" if config_file is None else config_file), **kwargs)
        # 继承来的属性
        self.Name = "ZarrDB"
        return
    ...
    def getTable(self, table_name, args={}):
        ...
        if not os.path.isdir(self.MainDir + os.sep + table_name): raise __QS_Error__(
            "ZarrDB.getTable: 表 '%s' 不存在!" % table_name)
    ...
    def renameFactor(self, table_name, old_factor_name, new_factor_name):
        ...
        with self._DataLock:
            ...
            iZTable = zarr.open(self.MainDir + os.sep + table_name, mode="a")
            if old_factor_name not in iZTable: raise __QS_Error__(
                "ZarrDB.renameFactor: 表 ’%s' 中不存在因子 '%s'!" % (table_name, old_factor_name))
            if new_factor_name in iZTable: raise __QS_Error__(
                "ZarrDB.renameFactor: 表 ’%s' 中的因子 '%s' 已存在!" % (table_name, new_factor_name))
    ...
    def _updateFactorData(self, factor_data, table_name, ifactor_name, data_type):
        ...
        with self._DataLock:
            ...
            IDIndices = pd.Series(np.arange(ZFactor["ID"].shape[0]), index=np.r_[OldIDs, NewIDs]).loc[
                factor_data.columns].tolist()
            DTIndices = pd.Series(np.arange(ZFactor["DateTime"].shape[0]), index=np.r_[OldDateTimes, NewDateTimes]).loc[
    ...
    def writeFactorData(self, factor_data, table_name, ifactor_name, if_exists="update", data_type=None):
        ...
        with self._DataLock:
            ...
            if ifactor_name not in ZTable:
                ...
                ZFactor.create_dataset("ID", shape=(factor_data.shape[1],), data=factor_data.columns.values,
                                       dtype=object, object_codec=numcodecs.VLenUTF8(), overwrite=True)
                ZFactor.create_dataset("DateTime", shape=(factor_data.shape[0],), data=factor_data.index.values,
                                       dtype="M8[ns]", overwrite=True)
                if data_type == "double":
                    ZFactor.create_dataset("Data", shape=factor_data.shape, data=factor_data.values, dtype="f8",
                                           fill_value=np.nan, overwrite=True)
                elif data_type == "string":
                    ZFactor.create_dataset("Data", shape=factor_data.shape, data=factor_data.values, dtype=object,
                                           object_codec=numcodecs.VLenUTF8(), overwrite=True)
                elif data_type == "object":
                    ZFactor.create_dataset("Data", shape=factor_data.shape, data=factor_data.values, dtype=object,
                                           object_codec=numcodecs.Pickle(), overwrite=True)
