...
class _WideTable(FactorTable):
    ...
    def getFactorMetaData(self, factor_names=None, key=None, args={}):
        ...
        if key == "DataType":
            if args.get("因子值类型", self.ValueType) == "scalar":
                return self._DataType.append(pd.Series(["string", "string"], index=["datetime", "code"])).loc[
                    factor_names]
            else:
                return pd.Series(["object"] * len(factor_names), index=factor_names)
        if key is None:
            return pd.DataFrame(self.getFactorMetaData(factor_names=factor_names, key="DataType", args=args),
                                columns=["DataType"])
        else:
    ...
    def getID(self, ifactor_name=None, idt=None, args={}):
        ...
        if idt is not None: SQLStr += "AND " + self._DBTableName + "." + DTField + "='" + idt.strftime(
            "%Y-%m-%d %H:%M:%S.%f") + "' "
    ...
    def getDateTime(self, ifactor_name=None, iid=None, start_dt=None, end_dt=None, args={}):
        ...
        if start_dt is not None: SQLStr += "AND " + self._DBTableName + "." + DTField + ">='" + start_dt.strftime(
            "%Y-%m-%d %H:%M:%S.%f") + "' "
        if end_dt is not None: SQLStr += "AND " + self._DBTableName + "." + DTField + "<='" + end_dt.strftime(
            "%Y-%m-%d %H:%M:%S.%f") + "' "
        if ifactor_name is not None: SQLStr += "AND " + self._DBTableName + "." + ifactor_name + " IS NOT NULL "
        ...
        if self._FactorDB.DBType != "sqlite3":
            ...
            return [dt.datetime.strptime(iRslt[0], "%Y-%m-%d %H:%M:%S.%f") for iRslt in self._FactorDB.fetchall(SQLStr)]
    ...
    def __QS_genGroupInfo__(self, factors, operation_mode):
        ...
        for iFactor in factors:
            ...
            if iArgConditions not in ArgConditionGroup:
                ...
                ArgConditionGroup[iArgConditions]["StartDT"] = min(operation_mode._FactorStartDT[iFactor.Name],
                                                                   ArgConditionGroup[iArgConditions]["StartDT"])
                ArgConditionGroup[iArgConditions]["args"]["回溯天数"] = max(
                    ArgConditionGroup[iArgConditions]["args"]["回溯天数"], iFactor.LookBack)
        ...
        for iArgConditions in ArgConditionGroup:
            ...
            Groups.append((self, ArgConditionGroup[iArgConditions]["FactorNames"],
                           list(ArgConditionGroup[iArgConditions]["RawFactorNames"]),
                           operation_mode.DTRuler[StartInd:EndInd + 1], ArgConditionGroup[iArgConditions]["args"]))
        return Groups
    ...
    def __QS_prepareRawData__(self, factor_names, ids, dts, args={}):
        ...
        if StartDate is not None:
            ...
            SQLStr += "WHERE " + self._DBTableName + "." + DTField + " IS NOT NULL "
        if ids is not None:
            SQLStr += "AND (" + genSQLInCondition(self._DBTableName + "." + IDField, ids, is_str=True,
                                                  max_num=1000) + ") "
        FilterStr = args.get("筛选条件", self.FilterCondition)
        if FilterStr: SQLStr += "AND " + FilterStr.format(Table=self._DBTableName) + " "
        ...
        if self._FactorDB.DBType == "sqlite3": RawData["QS_DT"] = [dt.datetime.strptime(iDT, "%Y-%m-%d %H:%M:%S.%f") for
                                                                   iDT in RawData.pop("QS_DT")]
        return RawData
            ...
            if NullIDs:
                ...
                if NullRawData:
                    NullRawData = pd.DataFrame(np.array(NullRawData, dtype="O"),
                                               columns=["QS_DT", "ID", "QS_Factor", "QS_FactorValue"])
                    RawData = pd.concat([NullRawData, RawData], ignore_index=True)
...
class _FeatureTable(_WideTable):
    ...
    def __QS_calcData__(self, raw_data, factor_names, ids, dts, args={}):
        ...
        Data = Data.iloc[:, 0, :]
        return pd.Panel(Data.values.T.reshape((Data.shape[1], Data.shape[0], 1)).repeat(len(dts), axis=2),
...
class SQLDB(QSSQLObject, WritableFactorDB):
    ...
    def __init__(self, sys_args={}, config_file=None, **kwargs):
        super().__init__(sys_args=sys_args, config_file=(
            __QS_ConfigPath__ + os.sep + "SQLDBConfig.json" if config_file is None else config_file), **kwargs)
    ...
    def connect(self):
        ...
        if self._Connector == "sqlite3":
            ...
            SQLStr = (
                    "SELECT TABLE_NAME, COLUMN_NAME, DATA_TYPE FROM information_schema.COLUMNS WHERE table_schema='%s' " % self.DBName)
            ...
            if not Rslt:
                ...
                self._TableFieldDataType = \
                    pd.DataFrame(np.array(Rslt), columns=["表", "因子", "DataType"]).set_index(["表", "因子"])["DataType"]
                self._TableFactorDict = self._TableFieldDataType.copy()

                Mask = (self._TableFactorDict.str.contains("char") | self._TableFactorDict.str.contains(
                    "date") | self._TableFactorDict.str.contains("text"))
                self._TableFactorDict[Mask] = "string"
                self._TableFactorDict[~Mask] = "double"
                self._TableFactorDict = {iTable[nPrefix:]: self._TableFactorDict.loc[iTable] for iTable in
                                         self._TableFactorDict.index.levels[0]}
                self._TableFieldDataType = {iTable[nPrefix:]: self._TableFieldDataType.loc[iTable] for iTable in
                                            self._TableFieldDataType.index.levels[0]}
        return 0
    ...
    def createTable(self, table_name, field_types):
        ...
        self.createDBTable(self.InnerPrefix + table_name, FieldTypes, primary_keys=["datetime", "code"],
                           index_fields=["datetime", "code"])
        self._TableFactorDict[table_name] = pd.Series(
            {iFactorName: ("string" if field_types[iFactorName].find("char") != -1 else "double") for iFactorName in
             field_types})
    ...
    def renameFactor(self, table_name, old_factor_name, new_factor_name):
        ...
        if (new_factor_name != old_factor_name) and (new_factor_name in self._TableFactorDict[table_name]):
            Msg = ("因子库 '%s' 调用方法 renameFactor 错误: 新因子名 '%s' 已经存在于因子表 '%s' 中!" % (
        ...
        self._TableFieldDataType[table_name][new_factor_name] = self._TableFieldDataType[table_name].pop(
            old_factor_name)
        return 0
    ...
    def deleteData(self, table_name, ids=None, dts=None, dt_ids=None):
        ...
        if dt_ids is not None:
            dt_ids = ["('" + iDTIDs[0].strftime("%Y-%m-%d %H:%M:%S.%f") + "', '" + iDTIDs[1] + "')" for iDTIDs in
                      dt_ids]
            SQLStr += "AND " + genSQLInCondition("(" + DBTableName + ".datetime, " + DBTableName + ".code)", dt_ids,
        ...
        for i in range(data.shape[0]):
            ...
            if iDataLen > 0:
                iData = data.iloc[i].apply(
                    lambda x: [None] * (iDataLen - len(x)) + x if isinstance(x, list) else [x] * iDataLen).tolist()
    ...
    def writeData(self, data, table_name, if_exists="update", data_type={}, **kwargs):
        if table_name not in self._TableFactorDict:
            FieldTypes = {iFactorName: _identifyDataType(self.DBType, data.iloc[i].dtypes) for i, iFactorName in
            ...
            if NewFactorNames:
                FieldTypes = {iFactorName: _identifyDataType(self.DBType, data.iloc[i].dtypes) for i, iFactorName in
                              enumerate(NewFactorNames)}
                self.addFactor(table_name, FieldTypes)
            AllFactorNames = self._TableFactorDict[table_name].index.tolist()
            if self.CheckWriteData:
                OldData = self.getTable(table_name, args={"因子值类型": "list", "时间转字符串": True}).readData(
                    factor_names=AllFactorNames, ids=data.minor_axis.tolist(), dts=data.major_axis.tolist())
            else:
                OldData = self.getTable(table_name, args={"时间转字符串": True}).readData(factor_names=AllFactorNames,
                                                                                    ids=data.minor_axis.tolist(),
                                                                                    dts=data.major_axis.tolist())
            if if_exists == "append":
                for iFactorName in AllFactorNames:
                    if iFactorName in data:
                        data[iFactorName] = OldData[iFactorName].where(pd.notnull(OldData[iFactorName]),
                                                                       data[iFactorName])
                    else:
