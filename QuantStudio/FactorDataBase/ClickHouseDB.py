...
class _WideTable(FactorTable):
    ...
    def getFactorMetaData(self, factor_names=None, key=None, args={}):
        ...
        if key == "DataType": return self._DataType.append(pd.Series(["string", "string"], index=["dt", "code"])).loc[
            factor_names]
        if key is None:
            return pd.DataFrame(
                self._DataType.append(pd.Series(["string", "string"], index=["dt", "code"])).loc[factor_names],
                columns=["DataType"])
        else:
...
class ClickHouseDB(SQLDB):
    ...
    def __init__(self, sys_args={}, config_file=None, **kwargs):
        super().__init__(sys_args=sys_args, config_file=(
            __QS_ConfigPath__ + os.sep + "ClickHouseDBConfig.json" if config_file is None else config_file), **kwargs)
        ...
        if (self.Connector == "clickhouse-driver") or ((self.Connector == "default") and (self.DBType == "ClickHouse")):
            try:
                ...
                if self.DSN:
                    ...
                    self._Connection = clickhouse_driver.connect(user=self.User, password=self.Pwd, host=self.IPAddr,
                                                                 port=self.Port, database=self.DBName)
            except Exception as e:
                Msg = ("'%s' 尝试使用 clickhouse-driver 连接(%s@%s:%d)数据库 '%s' 失败: %s" % (
                self.Name, self.User, self.IPAddr, self.Port, self.DBName, str(e)))
    ...
    def renameDBTable(self, old_table_name, new_table_name):
        ...
        try:
            ...
            self._QS_Logger.info(
                "'%s' 调用方法 renameDBTable 将表 '%s' 重命名为 '%s'" % (self.Name, old_table_name, new_table_name))
        return 0
    ...
    def createTable(self, table_name, field_types):
        ...
        self.createDBTable(self.InnerPrefix + table_name, FieldTypes, primary_keys=["dt", "code"],
                           index_fields=["dt", "code"])
        self._TableFactorDict[table_name] = pd.Series(
            {iFactorName: ("string" if field_types[iFactorName].find("String") != -1 else "double") for iFactorName in
             field_types})
    ...
    def addField(self, table_name, field_types):
        ...
        try:
            ...
            self._QS_Logger.info(
                "'%s' 调用方法 addField 为表 '%s' 添加字段 ’%s'" % (self.Name, table_name, str(list(field_types.keys()))))
        return 0
