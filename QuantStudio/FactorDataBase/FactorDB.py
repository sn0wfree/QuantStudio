        ...
        with ProgressBar(max_value=nTask) as ProgBar:
            for i, iTask in enumerate(TaskDispatched):
                ...
                if hasattr(iDB, "writeFactorData"):
                    ...
                    iDataTypes = {iTargetFactorNames[j]: jFactor.getMetaData(key="DataType") for j, jFactor in
                                  enumerate(iFactors)}
...
class FactorTable(__QS_Object__):
    ...
    def readData(self, factor_names, ids, dts, args={}):
        if self.ErgodicMode._isStarted: return self._readData_ErgodicMode(factor_names=factor_names, ids=ids, dts=dts,
                                                                          args=args)
        ...
        for iFactorName in factor_names:
            ...
            if iFactorData is None:  # 尚未进入缓存
                if self.ErgodicMode._CacheFactorNum < self.ErgodicMode.MaxFactorCacheNum:  # 当前缓存因子数小于最大缓存因子数，那么将该因子数据读入缓存
                    ...
                    if CacheFactorReadNum.loc[MinReadNumInd] < self.ErgodicMode._FactorReadNum[
                        iFactorName]:  # 当前读取的因子的读取次数超过了缓存因子读取次数的最小值，缓存该因子数据
                        CacheFactorNames.add(iFactorName)
    ...
    def _readIDData(self, iid, factor_names, dts, args={}):
        ...
        if IDData is None:  # 尚未进入缓存
            if self.ErgodicMode._CacheIDNum < self.ErgodicMode.MaxIDCacheNum:  # 当前缓存 ID 数小于最大缓存 ID 数，那么将该 ID 数据读入缓存
                ...
                if CacheIDReadNum.loc[MinReadNumInd] < self.ErgodicMode._IDReadNum[
                    iid]:  # 当前读取的 ID 的读取次数超过了缓存 ID 读取次数的最小值，缓存该 ID 数据
                    IDData = self.__QS_calcData__(
                        raw_data=self.__QS_prepareRawData__(factor_names=self.FactorNames, ids=[iid],
                                                            dts=self.ErgodicMode._CacheDTs, args=args),
    ...
    def _readData_ErgodicMode(self, factor_names, ids, dts, args={}):
        if self.ErgodicMode.CacheMode == "因子": return self._readData_FactorCacheMode(factor_names=factor_names, ids=ids,
                                                                                     dts=dts, args=args)
        return pd.Panel(
    ...
    def start(self, dts, **kwargs):
        ...
        if self.ErgodicMode._DateTimes.shape[0] == 0: raise __QS_Error__(
            "因子表: '%s' 的默认时间序列为空, 请设置参数 '遍历模式-遍历时点' !" % self.Name)
        ...
        self.ErgodicMode._FactorReadNum = pd.Series(0,
                                                    index=self.FactorNames)  # 因子读取次数, pd.Series(读取次数, index=self.FactorNames)
        ...
        if self.ErgodicMode.CacheSize > 0:
            ...
            if os.name == "nt": self._MMAPCacheData = mmap.mmap(-1, int(self.ErgodicMode.CacheSize * 2 ** 20),
                                                                tagname=self.ErgodicMode._TagName)  # 当前共享内存缓冲区
    ...
    def move(self, idt, **kwargs):
        ...
        if (self.ErgodicMode.CacheSize > 0) and (self.ErgodicMode._CurInd > -1) and (
                ...
                self.ErgodicMode._DateTimes[self.ErgodicMode._CurInd] > self.ErgodicMode._CacheDTs[-1])):  # 需要读入缓冲区的数据
        ...
        if isinstance(raw_data, pd.DataFrame) and ("ID" in raw_data):  # 如果原始数据有 ID 列，按照 ID 列划分后存入子进程的原始文件中
            ...
            for iPID, iIDs in pid_ids.items():
                with shelve.open(raw_data_dir + os.sep + iPID + os.sep + file_name) as iFile:
                    ...
                    if factor_names:
                        for jFactorName in factor_names: iFile[jFactorName] = iData[
                            CommonCols + [jFactorName]].reset_index()
                    else:
    ...
    def _genFactorDict(self, factors, factor_dict={}):
        for iFactor in factors:
            ...
            if (not isinstance(iFactor.Name, str)) or (iFactor.Name == "") or (
                    iFactor is not factor_dict.get(iFactor.Name, iFactor)):  # 该因子命名错误或者未命名, 或者有因子重名
        ...
        for iFTID, iGroups in InitGroups.items():
            ...
            for j in range(len(iGroups)):
                ...
                for k in range(ijGroupNum):
                    ...
                    for m in range(len(ijGroupInfo[k][1])): self.OperationMode._FactorDict[
                        ijGroupInfo[k][1][m]]._RawDataFile = ijkRawDataFileName
...
class CustomFT(FactorTable):
    ...
    def __init__(self, name, sys_args={}, config_file=None, **kwargs):
        ...
        self._FactorDict = pd.DataFrame(columns=["FTID", "ArgIndex", "NameInFT", "DataType"],
                                        dtype=np.dtype("O"))  # 数据源中因子的来源信息, index=[因子名]
    ...
    def renameFactor(self, factor_name, new_factor_name):
        ...
        if (new_factor_name != factor_name) and (new_factor_name in self._Factors): raise __QS_Error__(
            "因子: '%s' 有重名!" % new_factor_name)
...
class Factor(__QS_Object__):
    ...
    def getID(self, idt=None):
        ...
        if self._FactorTable is not None: return self._FactorTable.getID(ifactor_name=self._NameInFT, idt=idt,
                                                                         args=self.Args)
        return []
    ...
    def getDateTime(self, iid=None, start_dt=None, end_dt=None):
        ...
        if self._FactorTable is not None: return self._FactorTable.getDateTime(ifactor_name=self._NameInFT, iid=iid,
    ...
    def readData(self, ids, dts, **kwargs):
        if not self._isStarted: return \
            self._FactorTable.readData(factor_names=[self._NameInFT], ids=ids, dts=dts, args=self.Args).loc[
                self._NameInFT]
        # 启动了遍历模式
        ...
        if NewDTs:
            NewCacheData = \
                self._FactorTable.readData(factor_names=[self._NameInFT], ids=self._CacheData.columns.tolist(),
                                           dts=NewDTs,
                                           args=self.Args).loc[self._NameInFT]
            self._CacheData = self._CacheData.append(NewCacheData).loc[dts]
        NewIDs = sorted(set(ids).difference(self._CacheData.columns))
        if NewIDs:
            NewCacheData = \
                self._FactorTable.readData(factor_names=[self._NameInFT], ids=NewIDs,
    ...
    def __QS_prepareCacheData__(self, ids=None):
        ...
        if os.path.isfile(RawDataFilePath + self._OperationMode._FileSuffix):
            ...
            if RawData is not None:
                StdData = \
                    self._FactorTable.__QS_calcData__(RawData, factor_names=[self._NameInFT], ids=PrepareIDs, dts=DTs,
                                                      args=self.Args).iloc[0]
            else:
                StdData = \
                    self._FactorTable.readData(factor_names=[self._NameInFT], ids=PrepareIDs, dts=DTs,
                                               args=self.Args).iloc[
                        0]
        else:
            ...
            StdData = \
                self._FactorTable.readData(factor_names=[self._NameInFT], ids=PrepareIDs, dts=DTs, args=self.Args).iloc[
                    0]
