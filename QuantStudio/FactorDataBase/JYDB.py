...
import re
import shelve
from traits.api import Enum, Int, Str, Range, Bool, List, ListStr, ListInt, Dict, Function, Password, Either, Float, \
...
def _importInfo(info_file, info_resource, logger, out_info=False):
    ...
    if Suffix in ("xlsx", "xls"):
        ...
        ExchangeInfo = pd.read_excel(info_resource, "ExchangeInfo", dtype={"ExchangeCode": "O"}).set_index(
            ["ExchangeCode"])
    elif Suffix == "json":
...
def _updateInfo(info_file, info_resource, logger, out_info=False):
    ...
    if not os.path.isfile(info_file):
        ...
        try:
            ...
            return (
            readNestedDictFromHDF5(info_file, ref="/TableInfo"), readNestedDictFromHDF5(info_file, ref="/FactorInfo"),
...
class _DBTable(FactorTable):
    ...
    def __init__(self, name, fdb, sys_args={}, **kwargs):
        ...
        self._IDFieldIsStr = (_identifyDataType(
            self._FactorInfo["DataType"][self._FactorInfo["FieldType"] == "ID"].iloc[0]) != "double")
        self._ConditionFields = self._FactorInfo[
            self._FactorInfo["FieldType"] == "Condition"].index.tolist()  # 所有的条件字段列表
        ...
        if pd.isnull(self._MainTableName):
            ...
            self._JoinCondition = fdb._TableInfo.loc[name, "JoinCondition"].format(DBTable=self._DBTableName,
                                                                                   MainTable=self._MainTableName)
    ...
    def _getIDField(self):
        ...
        for iCode in ExchangeCodes:
            IDField += "WHEN " + iCode + " THEN CONCAT(" + RawIDField + ", '" + ExchangeInfo.loc[
                iCode, "Suffix"] + "') "
        ...
        for iField in RelatedFields.index:
            ...
            if iSQLStr[0] == "{":
                ...
                iMapInfo = self._FactorDB.fetchall(
                    iSQLStr.format(TablePrefix=self._FactorDB.TablePrefix, Keys=Keys, KeyCondition=KeyCondition,
                                   SecuCode=SecuCode))
            ...
            if isinstance(iInfo, str) and (iInfo.find("从表") != -1):
                ...
                if len(iInfo) == 2:
                    ...
                    if iSETableAlias not in SETables:
                        JoinStr.append(
                            "LEFT JOIN " + iSETable + " " + iSETableAlias + " ON (" + self._DBTableName + ".ID=" + iSETableAlias + ".ID AND " + iSETableAlias + ".TypeCode=" +
                            iInfo[1] + ")")
    ...
    def _genConditionSQLStr(self, args={}):
        ...
        if FilterStr:
            SQLStr = "AND " + FilterStr.format(Table=self._DBTableName, TablePrefix=self._FactorDB.TablePrefix) + " "
        else:
            SQLStr = ""
        for iConditionField in self._ConditionFields:
            ...
            if iConditionVal:
                ...
                if _identifyDataType(self._FactorInfo.loc[iConditionField, "DataType"]) != "double":
                    SQLStr += "AND " + self._DBTableName + "." + self._FactorInfo["DBFieldName"].loc[
                        iConditionField] + " IN ('" + "','".join(iConditionVal) + "') "
                else:
                    SQLStr += "AND " + self._DBTableName + "." + self._FactorInfo["DBFieldName"].loc[
                        iConditionField] + " IN (" + iConditionVal + ") "
        return SQLStr[:-1]
    ...
    def __QS_genGroupInfo__(self, factors, operation_mode):
        ...
        for iFactor in factors:
            iConditions = ";".join(
                [iCondition + ":" + str(iFactor[iCondition]) for i, iCondition in enumerate(self._ConditionFields)] + [
                    "筛选条件:" + iFactor["筛选条件"]])
            if iConditions not in ConditionGroup:
                ...
                ConditionGroup[iConditions]["StartDT"] = min(operation_mode._FactorStartDT[iFactor.Name],
        ...
        for iConditions in ConditionGroup:
            ...
            Groups.append((self, ConditionGroup[iConditions]["FactorNames"],
                           ...
                           operation_mode.DTRuler[StartInd:EndInd + 1], ConditionGroup[iConditions]["args"]))
        return Groups
...
class _FeatureTable(_DBTable):
    ...
    def __QS_genGroupInfo__(self, factors, operation_mode):
        ...
        for iFactor in factors:
            iConditions = ";".join(
                [iArgName + ":" + str(iFactor[iArgName]) for iArgName in iFactor.ArgNames if iArgName not in ("算子",)])
            if iConditions not in ConditionGroup:
                ConditionGroup[iConditions] = {"FactorNames": [iFactor.Name],
                                               "RawFactorNames": {iFactor._NameInFT},
                                               "StartDT": operation_mode._FactorStartDT[iFactor.Name],
    ...
    def __QS_calcData__(self, raw_data, factor_names, ids, dts, args={}):
        ...
        if raw_data.index.intersection(ids).shape[0] == 0: return pd.Panel(items=factor_names, major_axis=dts,
                                                                           minor_axis=ids)
        ...
        if args.get("多重映射", self.MultiMapping):
            ...
            if np.any(DupMask):
                ...
                Data = Data[~DupMask]
        Data = pd.Panel(Data.values.T.reshape((Data.shape[1], Data.shape[0], 1)).repeat(len(dts), axis=2),
                        items=factor_names, major_axis=Data.index, minor_axis=dts).swapaxes(1, 2)
        return Data.loc[:, :, ids]
...
class _MappingTable(_DBTable):
    ...
    def getID(self, ifactor_name=None, idt=None, args={}):
        ...
        SQLStr += self._genFromSQLStr() + " "
        if idt is not None:
            SQLStr += "WHERE " + self._DBTableName + "." + self._StartDateField + "<='" + idt.strftime(
                "%Y-%m-%d") + "' "
            if self._EndDateIncluded:
                SQLStr += "AND " + self._DBTableName + "." + self._EndDateField + ">='" + idt.strftime(
                    "%Y-%m-%d") + "' "
            else:
                SQLStr += "AND " + self._DBTableName + "." + self._EndDateField + ">'" + idt.strftime("%Y-%m-%d") + "' "
        else:
            SQLStr += "WHERE " + self._DBTableName + "." + self._StartDateField + " IS NOT NULL "
    ...
    def __QS_prepareRawData__(self, factor_names, ids, dts, args={}):
        ...
        SQLStr += "WHERE (" + genSQLInCondition(self._MainTableName + "." + self._MainTableID, deSuffixID(ids),
                                                is_str=self._IDFieldIsStr, max_num=1000) + ") "
        if pd.notnull(self._MainTableCondition): SQLStr += "AND " + self._MainTableCondition + " "
        SQLStr += "AND ((" + self._DBTableName + "." + self._EndDateField + ">='" + StartDate.strftime(
            "%Y-%m-%d") + "') "
    ...
    def _calcMultiMappingData(self, raw_data, factor_names, ids, dts, args={}):
        ...
        if args.get("只填起始日", self.OnlyStartFilled):
            ...
            for iID in raw_data.index.unique():
                ...
                iTempData = pd.DataFrame([([],) * nFactor] * len(iStartEndDates), index=iStartEndDates,
                                         columns=factor_names, dtype="O")
                ...
                for jStartDate, jEndDate in iRawData.index.drop_duplicates():
                    ...
                    if jEndDate < jStartDate:
                        ...
                        iTempData.loc[jStartDate:] += pd.DataFrame([ijRawData] * ijOldData.shape[0],
                                                                   index=ijOldData.index, columns=ijOldData.columns,
                                                                   dtype="O")
                    else:
                        jEndDate -= dt.timedelta(1)
                        ijOldData = iTempData.loc[jStartDate:jEndDate]
                        iTempData.loc[jStartDate:jEndDate] += pd.DataFrame([ijRawData] * ijOldData.shape[0],
                                                                           index=ijOldData.index,
                                                                           columns=ijOldData.columns, dtype="O")
            ...
            for iID in raw_data.index.unique():
                ...
                for jStartDate, jEndDate in iRawData.index.drop_duplicates():
                    ...
                    if jEndDate < jStartDate:
                        ...
                        iData.loc[jStartDate:] += pd.DataFrame([ijRawData] * ijOldData.shape[0], index=ijOldData.index,
                                                               columns=ijOldData.columns, dtype="O")
                    else:
                        jEndDate -= DeltaDT
                        ijOldData = iData.loc[jStartDate:jEndDate]
                        iData.loc[jStartDate:jEndDate] += pd.DataFrame([ijRawData] * ijOldData.shape[0],
                                                                       index=ijOldData.index, columns=ijOldData.columns,
                                                                       dtype="O")
                Data[iID] = iData
            return pd.Panel(Data).swapaxes(0, 2).loc[:, :, ids]
    ...
    def __QS_calcData__(self, raw_data, factor_names, ids, dts, args={}):
        ...
        if args.get("多重映射", self.MultiMapping): return self._calcMultiMappingData(raw_data, factor_names, ids, dts,
                                                                                  args=args)
        raw_data.set_index(["ID"], inplace=True)
        ...
        if args.get("只填起始日", self.OnlyStartFilled):
            ...
            for iID in raw_data.index.unique():
                ...
                for j in range(iRawData.shape[0]):
                    ...
                    if jEndDate < jStartDate:
                        iTempData.loc[jStartDate:] = np.repeat(ijRawData[factor_names].values.reshape((1, nFactor)),
                                                               iTempData.loc[jStartDate:].shape[0], axis=0)
                    else:
                        jEndDate -= dt.timedelta(1)
                        iTempData.loc[jStartDate:jEndDate] = np.repeat(
                            ijRawData[factor_names].values.reshape((1, nFactor)),
                            iTempData.loc[jStartDate:jEndDate].shape[0], axis=0)
                iData = pd.DataFrame(index=dts, columns=factor_names)
                for j, jDate in enumerate(iStartEndDates):
                    jIdx = iData.index.searchsorted(jDate)
                    if jIdx < iData.shape[0]:
            ...
            for iID in raw_data.index.unique():
                ...
                for j in range(iRawData.shape[0]):
                    ...
                    if jEndDate < jStartDate:
                        iData.loc[jStartDate:] = np.repeat(ijRawData[factor_names].values.reshape((1, nFactor)),
                                                           iData.loc[jStartDate:].shape[0], axis=0)
                    else:
                        jEndDate -= DeltaDT
                        iData.loc[jStartDate:jEndDate] = np.repeat(ijRawData[factor_names].values.reshape((1, nFactor)),
                                                                   iData.loc[jStartDate:jEndDate].shape[0], axis=0)
                Data[iID] = iData
            return pd.Panel(Data).swapaxes(0, 2).loc[:, :, ids]
...
class _ConstituentTable(_DBTable):
    ...
    def __init__(self, name, fdb, sys_args={}, **kwargs):
        ...
        if self._CurSignField.shape[0] == 0:
            self._CurSignField = None
        else:
            self._CurSignField = self._CurSignField[0]
        return

    @property
    def FactorNames(self):
        if self._AllGroups is None:
            SQLStr = "SELECT DISTINCT " + self._DBTableName + "." + self._FactorInfo.loc[
                self._GroupField, "DBFieldName"] + " "
            SQLStr += "FROM " + self._DBTableName + " "
    ...
    def getFactorMetaData(self, factor_names=None, key=None, args={}):
        ...
        if key == "DataType":
            ...
            return pd.Series(["0 or nan: 非成分; 1: 是成分"] * len(factor_names), index=factor_names)
        elif key is None:
    ...
    def getID(self, ifactor_name=None, idt=None, args={}, **kwargs):
        ...
        if ifactor_name is not None:
            SQLStr += "WHERE " + self._DBTableName + "." + self._FactorInfo.loc[
                self._GroupField, "DBFieldName"] + "='" + ifactor_name + "' "
        else:
            SQLStr += "WHERE " + self._DBTableName + "." + self._FactorInfo.loc[
                self._GroupField, "DBFieldName"] + " IS NOT NULL "
        if idt is not None:
            idt = idt.strftime("%Y%m%d")
            SQLStr += "AND " + self._DBTableName + "." + self._FactorInfo.loc[
                self._InDateField, "DBFieldName"] + "<='" + idt + "' "
            if kwargs.get("is_current", True):
                SQLStr += "AND ((" + self._DBTableName + "." + self._FactorInfo.loc[
                    self._OutDateField, "DBFieldName"] + ">'" + idt + "') "
                if self._CurSignField:
                    SQLStr += "OR (" + self._DBTableName + "." + self._FactorInfo.loc[
                        self._CurSignField, "DBFieldName"] + "=1)) "
                else:
                    SQLStr += "OR (" + self._DBTableName + "." + self._FactorInfo.loc[
                        self._OutDateField, "DBFieldName"] + " IS NULL)) "
    ...
    def getDateTime(self, ifactor_name=None, iid=None, start_dt=None, end_dt=None, args={}):
        if iid is not None:
            SQLStr = "SELECT " + self._DBTableName + "." + self._FactorInfo.loc[
                self._InDateField, "DBFieldName"] + " "  # 纳入日期
            SQLStr += self._DBTableName + "." + self._FactorInfo.loc[self._OutDateField, "DBFieldName"] + " "  # 剔除日期
            SQLStr += self._genFromSQLStr() + " "
            SQLStr += "WHERE " + self._DBTableName + "." + self._FactorInfo.loc[
                self._InDateField, "DBFieldName"] + " IS NOT NULL "
            if ifactor_name is not None: SQLStr += "AND " + self._DBTableName + "." + self._FactorInfo.loc[
                self._GroupField, "DBFieldName"] + "='" + ifactor_name + "' "
            SQLStr += "AND " + self._MainTableName + "." + self._MainTableID + "='" + deSuffixID([iid])[0] + "' "
            if start_dt is not None:
                SQLStr += "AND ((" + self._DBTableName + "." + self._FactorInfo.loc[
                    self._OutDateField, "DBFieldName"] + ">'" + start_dt.strftime("%Y%m%d") + "') "
                SQLStr += "OR (" + self._DBTableName + "." + self._FactorInfo.loc[
                    self._OutDateField, "DBFieldName"] + " IS NULL))"
            if end_dt is not None:
                SQLStr += "AND " + self._DBTableName + "." + self._FactorInfo.loc[
                    self._InDateField, "DBFieldName"] + "<='" + end_dt.strftime("%Y%m%d") + "' "
            SQLStr += self._genConditionSQLStr(args=args) + " "
            ...
            for iStartDate, iEndDate in Data:
                ...
                if iEndDate is None:
                    ...
                    iEndDT = dt.datetime.strptime(iEndDate, "%Y%m%d")
                DateTimes = DateTimes.union(
                    getDateTimeSeries(start_dt=iStartDT, end_dt=iEndDT, timedelta=dt.timedelta(1)))
            return sorted(DateTimes)
        SQLStr = "SELECT MIN(" + self._DBTableName + "." + self._FactorInfo.loc[
            self._InDateField, "DBFieldName"] + ") "  # 纳入日期
        SQLStr += "FROM " + self._DBTableName + " "
        if ifactor_name is not None:
            SQLStr += "WHERE " + self._DBTableName + "." + self._FactorInfo.loc[
    ...
    def __QS_prepareRawData__(self, factor_names, ids, dts, args={}):
        ...
        SQLStr += "WHERE (" + genSQLInCondition(self._MainTableName + "." + self._MainTableID, deSuffixID(ids),
                                                is_str=self._IDFieldIsStr, max_num=1000) + ")                 "
        SQLStr += "AND (" + genSQLInCondition(self._DBTableName + "." + FieldDict[self._GroupField], factor_names,
                                              is_str=False, max_num=1000) + ") "
        SQLStr += "AND ((" + self._DBTableName + "." + FieldDict[self._OutDateField] + ">'" + StartDate.strftime(
            "%Y%m%d") + "') "
        SQLStr += "OR (" + self._DBTableName + "." + FieldDict[self._OutDateField] + " IS NULL)) "
        SQLStr += "AND " + self._DBTableName + "." + FieldDict[self._InDateField] + "<='" + EndDate.strftime(
            "%Y%m%d") + "' "
        SQLStr += self._genConditionSQLStr(args=args) + " "
        ...
        RawData = self._FactorDB.fetchall(SQLStr)
        if not RawData:
            RawData = pd.DataFrame(columns=["IndexID", "SecurityID", "InDate", "OutDate", "CurSign"])
        else:
            RawData = pd.DataFrame(np.array(RawData, dtype="O"),
                                   columns=["IndexID", "SecurityID", "InDate", "OutDate", "CurSign"])
        return RawData
        ...
        for iIndexID in factor_names:
            ...
            for jID in iData.columns:
                ...
                for k in range(jIDRawData.shape[0]):
                    ...
                    kEndDate = (jIDRawData["OutDate"].iloc[k].date() - dt.timedelta(1) if jIDRawData["OutDate"].iloc[
                                                                                              k] is not None else dt.date.today())
                    iData[jID].loc[kStartDate:kEndDate] = 1
            Data[iIndexID] = iData
        Data = pd.Panel(Data)
        if Data.minor_axis.intersection(ids).shape[0] == 0: return pd.Panel(0.0, items=factor_names, major_axis=dts,
                                                                            minor_axis=ids)
...
class _MarketTable(_DBTable):
    ...
    def __QS_initArgs__(self):
        ...
        iFactorInfo = self._FactorInfo[
            self._FactorInfo["FieldType"].str.contains("Date") & pd.notnull(self._FactorInfo["Supplementary"])]
        iFactorInfo = iFactorInfo[iFactorInfo["Supplementary"].str.contains("Default")]
        if iFactorInfo.shape[0] > 0:
            self.DateField = iFactorInfo.index[0]
        else:
            self.DateField = DateFields[0]
    ...
    def getID(self, ifactor_name=None, idt=None, args={}):
        ...
        SQLStr += self._genFromSQLStr() + " "
        if idt is not None:
            SQLStr += "WHERE " + DateField + "='" + idt.strftime("%Y-%m-%d") + "' "
        else:
            SQLStr += "WHERE " + DateField + " IS NOT NULL "
        if pd.notnull(self._MainTableCondition): SQLStr += "AND " + self._MainTableCondition + " "
    ...
    def __QS_genGroupInfo__(self, factors, operation_mode):
        ...
        for iFactor in factors:
            ...
            iDateConditions = (iFactor.DateField, ";".join(
                [iArgName + ":" + str(iFactor[iArgName]) for iArgName in iFactor.ArgNames if
                 iArgName not in ("回溯天数", "只起始日回溯", "只回溯非目标日", "只回溯时点")]))
            if iDateConditions not in DateConditionGroup:
                ...
                DateConditionGroup[iDateConditions]["StartDT"] = min(operation_mode._FactorStartDT[iFactor.Name],
                                                                     DateConditionGroup[iDateConditions]["StartDT"])
                DateConditionGroup[iDateConditions]["args"]["回溯天数"] = max(
                    DateConditionGroup[iDateConditions]["args"]["回溯天数"], iFactor.LookBack)
        EndInd = operation_mode.DTRuler.index(operation_mode.DateTimes[-1])
        Groups = []
        for iDateConditions in DateConditionGroup:
            ...
            Groups.append((self, DateConditionGroup[iDateConditions]["FactorNames"],
                           list(DateConditionGroup[iDateConditions]["RawFactorNames"]),
                           operation_mode.DTRuler[StartInd:EndInd + 1], DateConditionGroup[iDateConditions]["args"]))
        return Groups
    ...
    def _genNullIDSQLStr(self, factor_names, ids, end_date, args={}):
        ...
        SubSQLStr += "AND (" + genSQLInCondition(self._MainTableName + "." + self._MainTableID, deSuffixID(ids),
                                                 is_str=self._IDFieldIsStr, max_num=1000) + ") "
    ...
    def _genSQLStr(self, factor_names, ids, start_date, end_date, args={}):
        ...
        if pd.notnull(self._MainTableCondition): SQLStr += "AND " + self._MainTableCondition + " "
        SQLStr += "AND (" + genSQLInCondition(self._MainTableName + "." + self._MainTableID, deSuffixID(ids),
                                              is_str=self._IDFieldIsStr, max_num=1000) + ") "
        SQLStr += self._genConditionSQLStr(args=args) + " "
    ...
    def __QS_prepareRawData__(self, factor_names, ids, dts, args={}):
        ...
        RawData = self._FactorDB.fetchall(
            self._genSQLStr(factor_names, ids, start_date=StartDate, end_date=EndDate, args=args))
        ...
        if np.isinf(LookBack):
            NullIDs = set(ids).difference(
                set(RawData[RawData["日期"] == dt.datetime.combine(StartDate, dt.time(0))]["ID"]))
            if NullIDs:
                NullRawData = self._FactorDB.fetchall(
                    self._genNullIDSQLStr(factor_names, list(NullIDs), StartDate, args=args))
                if NullRawData:
...
class _InfoPublTable(_MarketTable):
    ...
    def getID(self, ifactor_name=None, idt=None, args={}):
        ...
        if self._AnnDateField is None:
            AnnDateField = EndDateField
        else:
        ...
        if AnnDateField != EndDateField:
            if idt is not None:
                SQLStr += "WHERE (CASE WHEN " + AnnDateField + ">=" + EndDateField + " THEN " + AnnDateField + " ELSE " + EndDateField + " END)='" + idt.strftime(
                    DTFormat) + "' "
            else:
    ...
    def getDateTime(self, ifactor_name=None, iid=None, start_dt=None, end_dt=None, args={}):
        ...
        EndDateField = self._DBTableName + "." + self._FactorDB._FactorInfo.loc[self.Name].loc[
            args.get("日期字段", self.DateField), "DBFieldName"]
        if self._AnnDateField is None:
            AnnDateField = EndDateField
        else:
            AnnDateField = self._DBTableName + "." + self._AnnDateField
    ...
    def _genNullIDSQLStr_InfoPubl(self, factor_names, ids, end_date, args={}):
        ...
        SubSQLStr += ConditionSQLStr + " "
        if (self._MainTableName is None) or (self._MainTableName == self._DBTableName):
            SubSQLStr += "AND (" + genSQLInCondition(self._DBTableName + "." + self._IDField, deSuffixID(ids),
                                                     is_str=self._IDFieldIsStr, max_num=1000) + ") "
    ...
    def __QS_prepareRawData__(self, factor_names, ids, dts, args={}):
        if self.IgnorePublDate or (self._AnnDateField is None): return super().__QS_prepareRawData__(
        ...
        if np.isinf(LookBack):
            ...
            if NullIDs:
                NullRawData = self._FactorDB.fetchall(
                    self._genNullIDSQLStr_InfoPubl(factor_names, list(NullIDs), StartDate, args=args))
                if NullRawData:
                    NullRawData = pd.DataFrame(np.array(NullRawData, dtype="O"), columns=["日期", "ID"] + factor_names)
                    RawData = pd.concat([NullRawData, RawData], ignore_index=True)
                    RawData.sort_values(by=["ID", "日期"])
...
class _NarrowTable(_DBTable):
    ...
    def __QS_initArgs__(self):
        ...
        iFactorInfo = self._FactorInfo[
            (self._FactorInfo["FieldType"] == "Date") & pd.notnull(self._FactorInfo["Supplementary"])]
        iFactorInfo = iFactorInfo[iFactorInfo["Supplementary"].str.contains("Default")]
        if iFactorInfo.shape[0] > 0:
            self.DateField = iFactorInfo.index[0]
        else:
        ...
        self.add_trait("FactorField",
                       Enum(*FactorFields.index.tolist(), arg_type="SingleOption", label="因子字段", order=2))
        DefaultField = FactorFields[FactorFields["Supplementary"] == "Default"].index
        if DefaultField.shape[0] == 0:
            self.FactorField = FactorFields.index[0]
        else:
            self.FactorField = DefaultField[0]
        ValueFields = self._FactorInfo[self._FactorInfo["FieldType"] == "Value"]
        self.add_trait("FactorValueField",
                       Enum(*ValueFields.index.tolist(), arg_type="SingleOption", label="因子值字段", order=3))
        DefaultField = ValueFields[ValueFields["Supplementary"] == "Default"].index
        if DefaultField.shape[0] == 0:
            self.FactorValueField = ValueFields.index[0]
        else:
            self.FactorValueField = DefaultField[0]
    ...
    def __QS_genGroupInfo__(self, factors, operation_mode):
        ...
        for iFactor in factors:
            ...
            iDateConditions = (self._DateField, ";".join(
                [iArgName + ":" + str(iFactor[iArgName]) for iArgName in iFactor.ArgNames if
                 iArgName not in ("回溯天数", "只起始日回溯")]))
            if iDateConditions not in DateConditionGroup:
                DateConditionGroup[iDateConditions] = {"FactorNames": [iFactor.Name],
                                                       "RawFactorNames": {iFactor._NameInFT},
                                                       "StartDT": operation_mode._FactorStartDT[iFactor.Name],
    ...
    def _genNullIDSQLStr(self, factor_names, ids, end_date, args={}):
        ...
        SQLStr += self._DBTableName + "." + self._FactorInfo.loc[
            args.get("因子值字段", self.FactorValueField), "DBFieldName"] + " "
        SQLStr += self._genFromSQLStr() + " "
        SQLStr += "WHERE (" + self._MainTableName + "." + self._MainTableID + ", " + DateField + ") IN (" + SubSQLStr + ") "
        if pd.notnull(self._MainTableCondition): SQLStr += "AND " + self._MainTableCondition + " "
        FactorNames = self._getFactorNames(FactorField, check_list=True)
        if isinstance(FactorNames, list):
            SQLStr += "AND (" + genSQLInCondition(DBFactorField, factor_names, is_str=FactorFieldStr,
                                                  max_num=1000) + ") "
        elif isinstance(FactorNames, dict):
            SQLStr += "AND (" + genSQLInCondition(DBFactorField, [FactorNames[iFactor] for iFactor in factor_names],
                                                  is_str=FactorFieldStr, max_num=1000) + ") "
        SQLStr += ConditionSQLStr
        return SQLStr
    ...
    def __QS_prepareRawData__(self, factor_names, ids, dts, args={}):
        ...
        if np.isinf(LookBack):
            NullIDs = set(ids).difference(
                set(RawData[RawData["QS_DT"] == dt.datetime.combine(StartDate, dt.time(0))]["ID"]))
            if NullIDs:
                NullRawData = self._FactorDB.fetchall(
                    self._genNullIDSQLStr(factor_names, list(NullIDs), StartDate, args=args))
                if NullRawData:
                    NullRawData = pd.DataFrame(np.array(NullRawData, dtype="O"),
                                               columns=["QS_DT", "ID", FactorField, FactorValueField])
                    RawData = pd.concat([NullRawData, RawData], ignore_index=True)
    ...
    def __QS_calcData__(self, raw_data, factor_names, ids, dts, args={}):
        ...
        return adjustDataDTID(Data, LookBack, factor_names, ids, dts, args.get("只起始日回溯", self.OnlyStartLookBack),
...
class _FinancialTable(_DBTable):
    ...
    ReportDate = Enum("所有", "定期报告", "年报", "中报", "一季报", "三季报", Dict(), Function(), label="报告期", arg_type="SingleOption",
                      order=0)
    ...
    def __init__(self, name, fdb, sys_args={}, **kwargs):
        ...
        if self._AnnDateField.shape[0] > 0:
            self._AnnDateField = self._AnnDateField[0]
        else:
            self._AnnDateField = None
        self._TempData = {}
        return
    ...
    def __QS_initArgs__(self):
        ...
        if self._AdjustTypeField.shape[0] == 0:
            self._AdjustTypeField = None
        else:
            self._AdjustTypeField = self._AdjustTypeField[0]
    ...
    def _genConditionSQLStr(self, args={}):
        ...
        if args.get("忽略非季末报告", self.IgnoreNonQuarter) or (not (
                (args.get("报告期", self.ReportDate) == "所有") and (args.get("计算方法", self.CalcType) == "最新") and (
                args.get("回溯年数", self.YearLookBack) == 0) and (args.get("回溯期数", self.PeriodLookBack) == 0))):
            if self._FactorDB.DBType == "SQL Server":
                ...
                raise __QS_Error__(
                    "JYDB._FinancialTable._genConditionSQLStr 不支持的数据库类型: '%s'" % (self._FactorDB.DBType,))
        if self._AdjustTypeField is not None:
            ...
            if iConditionVal:
                if _identifyDataType(self._FactorInfo.loc[self._AdjustTypeField, "DataType"]) != "double":
                    SQLStr += "AND " + self._DBTableName + "." + self._FactorInfo.loc[
                        self._AdjustTypeField, "DBFieldName"] + " IN ('" + "','".join(iConditionVal.split(",")) + "') "
                else:
                    SQLStr += "AND " + self._DBTableName + "." + self._FactorInfo.loc[
                        self._AdjustTypeField, "DBFieldName"] + " IN (" + iConditionVal + ") "
        return SQLStr

    def __QS_genGroupInfo__(self, factors, operation_mode):
        ConditionGroup = {}
        for iFactor in factors:
            iConditions = (iFactor.IgnoreNonQuarter or (not (
                        (iFactor.ReportDate == "所有") and (iFactor.CalcType == "最新") and (
                            iFactor.YearLookBack == 0) and (iFactor.PeriodLookBack == 0))))
            iConditions = (iConditions, iFactor.AdjustType, ";".join(
                [iCondition + ":" + str(iFactor[iCondition]) for i, iCondition in enumerate(self._ConditionFields)] + [
                    "筛选条件:" + iFactor["筛选条件"]]))
            if iConditions not in ConditionGroup:
                ConditionGroup[iConditions] = {"FactorNames": [iFactor.Name],
    ...
    def __QS_prepareRawData__(self, factor_names, ids, dts, args={}):
        ...
        ReportDateField = self._DBTableName + "." + self._FactorInfo.loc[self._DateField, "DBFieldName"]
        if self._AdjustTypeField is not None:
            AdjustTypeField = self._DBTableName + "." + self._FactorInfo.loc[self._AdjustTypeField, "DBFieldName"]
        else:
            AdjustTypeField = "2"
    ...
    def _calcData(self, raw_data, periods, factor_name, ids, dts, calc_type, report_date, ignore_missing):
        ...
        if report_date == "年报":
            TargetReportDate.loc[raw_data["ReportPeriod"] != "12-31", "ReportDate"] = dt.datetime(1899, 12, 31)
        elif report_date == "中报":
            TargetReportDate.loc[raw_data["ReportPeriod"] != "06-30", "ReportDate"] = dt.datetime(1899, 12, 31)
        elif report_date == "一季报":
            TargetReportDate.loc[raw_data["ReportPeriod"] != "03-31", "ReportDate"] = dt.datetime(1899, 12, 31)
        elif report_date == "三季报":
            TargetReportDate.loc[raw_data["ReportPeriod"] != "09-30", "ReportDate"] = dt.datetime(1899, 12, 31)
        TargetReportDate = TargetReportDate.set_index(["ID"]).groupby(level=0).cummax().reset_index().groupby(
            by=["ID", "AnnDate"], as_index=False).max()
        ...
        for i, iPeriod in enumerate(periods):
            ...
            iData = iData[(iData["AnnDate"] >= iData["AnnDate_y"]) | pd.isnull(iData["AnnDate_y"])].sort_values(
                by=["ID", "AnnDate", "AnnDate_y", "AdjustType"])
            if (i == 0) and (calc_type != "最新"):
                iData = iData.loc[:, ["ID", "AnnDate", "ReportPeriod", factor_name]].groupby(by=["ID", "AnnDate"],
                                                                                             as_index=True).last()
                ReportPeriod = iData.loc[:, "ReportPeriod"].where(pd.notnull(iData.loc[:, "ReportPeriod"]),
                                                                  "None").unstack().T
    ...
    def __QS_calcData__(self, raw_data, factor_names, ids, dts, args={}):
        if raw_data.shape[0] == 0: return pd.Panel(items=factor_names, major_axis=dts, minor_axis=ids)
        CalcType, YearLookBack, PeriodLookBack, ReportDate, IgnoreMissing = args.get("计算方法", self.CalcType), args.get(
            "回溯年数", self.YearLookBack), args.get("回溯期数", self.PeriodLookBack), args.get("报告期",
                                                                                        self.ReportDate), args.get(
            "忽略缺失", self.IgnoreMissing)
        if CalcType == "最新":
            Periods = np.array([0], dtype=np.int)
        elif CalcType == "单季度":
            Periods = np.array([0, 1], dtype=np.int)
        elif CalcType == "TTM":
            Periods = np.array([0, 1, 2, 3, 4], dtype=np.int)
        Periods += YearLookBack * 4 + PeriodLookBack
        raw_data["ReportPeriod"] = raw_data["ReportDate"].astype(str).str.slice(start=5)
        Data = {}
        for iFactorName in factor_names:
            Data[iFactorName] = self._calcData(
                raw_data.loc[:, ["ID", "AnnDate", "ReportDate", "AdjustType", "ReportPeriod", iFactorName]], Periods,
                iFactorName, ids, dts, CalcType, ReportDate, IgnoreMissing)
...
class _FinancialTable_Old(_DBTable):
    ...
    def _genConditionSQLStr(self, args={}):
        ...
        if self._AdjustTypeField is not None:
            ...
            if iConditionVal:
                ...
                SQLStr += " AND " + self._DBTableName + "." + self._FactorInfo.loc[
                    self._AdjustTypeField, "DBFieldName"] + " IN (" + ",".join(iConditionVal) + ")"
        return SQLStr
    ...
    def __QS_genGroupInfo__(self, factors, operation_mode):
        ...
        for iExprNoticeConditions in ExprNoticeConditionGroup:
            Groups.append((self, ExprNoticeConditionGroup[iExprNoticeConditions]["FactorNames"],
                           list(ExprNoticeConditionGroup[iExprNoticeConditions]["RawFactorNames"]), [],
                           ExprNoticeConditionGroup[iExprNoticeConditions]["args"]))
        return Groups
    ...
    def __QS_prepareRawData__(self, factor_names, ids, dts, args={}):
        ...
        AnnDateField = self._DBTableName + "." + self._FactorInfo.loc[self._AnnDateField, "DBFieldName"]
        ReportDateField = self._DBTableName + "." + self._FactorInfo.loc[self._DateField, "DBFieldName"]
        if self._AdjustTypeField is not None: AdjustTypeField = self._DBTableName + "." + self._FactorInfo.loc[
            self._AdjustTypeField, "DBFieldName"]
        # 形成 SQL 语句, ID, 公告日期, 报告期, 报表类型, 财务因子
        SQLStr = "SELECT " + self._getIDField() + " AS ID, "
        if self._FactorDB.DBType == "SQL Server":
        ...
        RawData = self._FactorDB.fetchall(SQLStr)
        if not RawData:
            return pd.DataFrame(columns=["ID", "AnnDate", "ReportDate"] + factor_names)
        else:
    ...
    def __QS_calcData__(self, raw_data, factor_names, ids, dts, args={}):
        ...
        if (YearLookBack == 0) and (PeriodLookBack == 0):
            if CalcType == "最新":
                CalcFun = self._calcIDData_LR
            elif CalcType == "单季度":
                CalcFun = self._calcIDData_SQ
            elif CalcType == "TTM":
                CalcFun = self._calcIDData_TTM
        elif YearLookBack > 0:
            if CalcType == "最新":
                CalcFun = self._calcIDData_LR_NYear
            elif CalcType == "单季度":
                CalcFun = self._calcIDData_SQ_NYear
            elif CalcType == "TTM":
                CalcFun = self._calcIDData_TTM_NYear
        elif PeriodLookBack > 0:
            if CalcType == "最新":
                CalcFun = self._calcIDData_LR_NPeriod
            elif CalcType == "单季度":
                CalcFun = self._calcIDData_SQ_NPeriod
            elif CalcType == "TTM":
                CalcFun = self._calcIDData_TTM_NPeriod
        raw_data = raw_data.set_index(["ID"])
        Data = {}
        for iID in raw_data.index.unique():
            Data[iID] = CalcFun(Dates, raw_data.loc[[iID]], factor_names, ReportDate, YearLookBack, PeriodLookBack,
                                IgnoreMissing)
        Data = pd.Panel(Data, major_axis=[dt.datetime.strptime(iDate, "%Y%m%d") for iDate in Dates],
                        minor_axis=factor_names).swapaxes(0, 2)
        NewData = {}
    ...
    def _findMaxReportDateInd(self, idate, raw_data, report_date, MaxReportDateInd, MaxNoteDateInd, PreMaxNoteDateInd):
        if isinstance(report_date, dict):
            ...
            Changed = False
            for i in range(0, MaxNoteDateInd - PreMaxNoteDateInd):
                if (MaxReportDateInd < 0) or (
                        raw_data['ReportDate'].iloc[MaxNoteDateInd - i] >= raw_data['ReportDate'].iloc[
                    MaxReportDateInd]):
                    MaxReportDateInd = MaxNoteDateInd - i
                    Changed = True
        elif report_date == '年报':
            Changed = False
            for i in range(0, MaxNoteDateInd - PreMaxNoteDateInd):
                if (raw_data['ReportDate'].iloc[MaxNoteDateInd - i][-4:] == '1231') and ((MaxReportDateInd < 0) or (
                        raw_data['ReportDate'].iloc[MaxNoteDateInd - i] >= raw_data['ReportDate'].iloc[
                    MaxReportDateInd])):
                    MaxReportDateInd = MaxNoteDateInd - i
                    Changed = True
            ...
            Changed = False
            for i in range(0, MaxNoteDateInd - PreMaxNoteDateInd):
                if (raw_data['ReportDate'].iloc[MaxNoteDateInd - i][-4:] == '0630') and ((MaxReportDateInd < 0) or (
                        raw_data['ReportDate'].iloc[MaxNoteDateInd - i] >= raw_data['ReportDate'].iloc[
                    MaxReportDateInd])):
                    MaxReportDateInd = MaxNoteDateInd - i
                    Changed = True
            ...
            Changed = False
            for i in range(0, MaxNoteDateInd - PreMaxNoteDateInd):
                if (raw_data['ReportDate'].iloc[MaxNoteDateInd - i][-4:] == '0331') and ((MaxReportDateInd < 0) or (
                        raw_data['ReportDate'].iloc[MaxNoteDateInd - i] >= raw_data['ReportDate'].iloc[
                    MaxReportDateInd])):
                    MaxReportDateInd = MaxNoteDateInd - i
                    Changed = True
            ...
            Changed = False
            for i in range(0, MaxNoteDateInd - PreMaxNoteDateInd):
                if (raw_data['ReportDate'].iloc[MaxNoteDateInd - i][-4:] == '0930') and ((MaxReportDateInd < 0) or (
                        raw_data['ReportDate'].iloc[MaxNoteDateInd - i] >= raw_data['ReportDate'].iloc[
                    MaxReportDateInd])):
                    MaxReportDateInd = MaxNoteDateInd - i
                    Changed = True
    ...
    def _calcIDData_LR(self, date_seq, raw_data, factor_names, report_date, year_lookback, period_lookback,
        ...
        for i, iDate in enumerate(date_seq):
            ...
            MaxReportDateInd, Changed = self._findMaxReportDateInd(iDate, raw_data, report_date, MaxReportDateInd,
                                                                   tempInd, tempPreInd)
            ...
            StdData[i] = \
            iRawData[iRawData["ReportDate"] == MaxReportDate][factor_names].fillna(method=FillnaMethod).values[-1]
    ...
    def _calcIDData_SQ(self, date_seq, raw_data, factor_names, report_date, year_lookback, period_lookback,
        ...
        for i, iDate in enumerate(date_seq):
            ...
            if MaxReportDate[-4:] == "1231":
                iPreReportDate = MaxReportDate[0:4] + "0930"
            elif MaxReportDate[-4:] == "0930":
                iPreReportDate = MaxReportDate[0:4] + "0630"
            elif MaxReportDate[-4:] == "0630":
                iPreReportDate = MaxReportDate[0:4] + "0331"
            else:
                StdData[i] = \
                iRawData[iRawData["ReportDate"] == MaxReportDate][factor_names].fillna(method=FillnaMethod).values[-1]
                continue
            iPreReportData = iRawData[iRawData["ReportDate"] == iPreReportDate][factor_names].fillna(
                method=FillnaMethod).values  # 前一个报告期数据
    ...
    def _calcIDData_TTM(self, date_seq, raw_data, factor_names, report_date, year_lookback, period_lookback,
        ...
        for i, iDate in enumerate(date_seq):
            ...
            if MaxReportDate[-4:] == '1231':  # 最新财报为年报
                ...
                iPreYearReport = iRawData[iRawData["ReportDate"] == iLastYear + "1231"][factor_names].fillna(
                    method=FillnaMethod).values  # 去年年报数据
                iPreReportData = iRawData[iRawData["ReportDate"] == iLastYear + MaxReportDate[-4:]][
                    factor_names].fillna(method=FillnaMethod).values  # 去年同期数据
                if (iPreReportData.shape[0] == 0) or (iPreYearReport.shape[0] == 0): continue
                StdData[i] = \
                iRawData[iRawData["ReportDate"] == MaxReportDate][factor_names].fillna(method=FillnaMethod).values[-1] + \
                iPreYearReport[-1] - iPreReportData[-1]
        self._TempData.pop("LastTargetReportDate", None)
        self._TempData.pop("LastTargetReportInd", None)
        return StdData
    ...
    def _calcIDData_LR_NYear(self, date_seq, raw_data, factor_names, report_date, year_lookback, period_lookback,
        ...
        for i, iDate in enumerate(date_seq):
            ...
            iRawData = raw_data.iloc[:tempInd + 1]
            iPreData = iRawData[iRawData["ReportDate"] == iLastNYear + MaxReportDate[-4:]][factor_names].fillna(
                method=FillnaMethod).values
    ...
    def _calcIDData_SQ_NYear(self, date_seq, raw_data, factor_names, report_date, year_lookback, period_lookback,
        ...
        for i, iDate in enumerate(date_seq):
            ...
            if MaxReportDate[-4:] == "1231":
                ...
                iPreReportData1 = iRawData[iRawData["ReportDate"] == iLastNYear + "0331"][factor_names].fillna(
                    method=FillnaMethod).values
                if iPreReportData1.shape[0] > 0: StdData[i] = iPreReportData1[-1]
                continue
            iPreReportData1 = iRawData[iRawData["ReportDate"] == iPreReportDate1][factor_names].fillna(
                method=FillnaMethod).values  # 上N年同期财报数据
            iPreReportData2 = iRawData[iRawData["ReportDate"] == iPreReportDate2][factor_names].fillna(
                method=FillnaMethod).values  # 上N年同期的上一期财报数据
    ...
    def _calcIDData_TTM_NYear(self, date_seq, raw_data, factor_names, report_date, year_lookback, period_lookback,
        ...
        for i, iDate in enumerate(date_seq):
            ...
            iRawData = raw_data.iloc[:tempInd + 1]
            if MaxReportDate[-4:] == "1231":  # 最新财报为年报
                iPreNReportData = iRawData[iRawData["ReportDate"] == str(iLastNYear) + "1231"][factor_names].fillna(
                    method=FillnaMethod).values
                if iPreNReportData.shape[0] > 0: StdData[i] = iPreNReportData[-1]
                continue
            iPreNReportData = iRawData[iRawData["ReportDate"] == str(iLastNYear) + MaxReportDate[-4:]][
                factor_names].fillna(method=FillnaMethod).values  # 上N年同期数据
            iPreN_1YearReportData = iRawData[iRawData["ReportDate"] == str(iLastNYear - 1) + "1231"][
                factor_names].fillna(method=FillnaMethod).values  # 上N+1年年报数据
            iPreN_1ReportData = iRawData[iRawData["ReportDate"] == str(iLastNYear - 1) + MaxReportDate[-4:]][
                factor_names].fillna(method=FillnaMethod).values  # 上N+1年同期数据
            if (iPreNReportData.shape[0] == 0) or (iPreN_1YearReportData.shape[0] == 0) or (
                    iPreN_1ReportData.shape[0] == 0): continue
    ...
    def _calcIDData_LR_NPeriod(self, date_seq, raw_data, factor_names, report_date, year_lookback, period_lookback,
        ...
        for i, iDate in enumerate(date_seq):
            ...
            iPreData = iRawData[iRawData["ReportDate"] == ObjectReportDate][factor_names].fillna(
                method=FillnaMethod).values
            if iPreData.shape[0] > 0: StdData[i] = iPreData[-1]
        self._TempData.pop("LastTargetReportDate", None)
        self._TempData.pop("LastTargetReportInd", None)
    ...
    def _calcIDData_SQ_NPeriod(self, date_seq, raw_data, factor_names, report_date, year_lookback, period_lookback,
        ...
        for i, iDate in enumerate(date_seq):
            ...
            if ObjectReportDate[-4:] == "1231":
                ...
                iPreReportData1 = iRawData[iRawData["ReportDate"] == ObjectReportDate][factor_names].fillna(
                    method=FillnaMethod).values
                if iPreReportData1.shape[0] > 0: StdData[i] = iPreReportData1[-1]
                continue
            iPreReportData1 = iRawData[iRawData["ReportDate"] == ObjectReportDate][factor_names].fillna(
                method=FillnaMethod).values  # 上N期财报数据
            iPreReportData2 = iRawData[iRawData["ReportDate"] == iPreReportDate][factor_names].fillna(
                method=FillnaMethod).values  # 上N+1期财报数据
            if (iPreReportData1.shape[0] == 0) or (iPreReportData2.shape[0] == 0): continue
            StdData[i] = iPreReportData1[-1] - iPreReportData2[-1]
        self._TempData.pop("LastTargetReportDate", None)
        self._TempData.pop("LastTargetReportInd", None)
    ...
    def _calcIDData_TTM_NPeriod(self, date_seq, raw_data, factor_names, report_date, year_lookback, period_lookback,
        ...
        for i, iDate in enumerate(date_seq):
            ...
            if ObjectReportDate[-4:] == '1231':  # 上N期财报为年报
                iPreNPeriodReportData = iRawData[iRawData["ReportDate"] == ObjectReportDate][factor_names].fillna(
                    method=FillnaMethod).values
                if iPreNPeriodReportData.shape[0] > 0: StdData[i] = iPreNPeriodReportData[-1]
                continue
            iPreNPeriodReportData = iRawData[iRawData["ReportDate"] == ObjectReportDate][factor_names].fillna(
                method=FillnaMethod).values  # 上N期数据
            iPreNPeriodYear_1YearReportData = \
            iRawData[iRawData["ReportDate"] == str(int(ObjectReportDate[0:4]) - 1) + "1231"][factor_names].fillna(
                method=FillnaMethod).values  # 上N期上一年年报数据
            iPreNPeriodYear_1ReportData = \
            iRawData[iRawData["ReportDate"] == str(int(ObjectReportDate[0:4]) - 1) + ObjectReportDate[-4:]][
                factor_names].fillna(method=FillnaMethod).values  # 上N期上一年同期数据
            if (iPreNPeriodReportData.shape[0] == 0) or (iPreNPeriodYear_1YearReportData.shape[0] == 0) or (
                    iPreNPeriodYear_1ReportData.shape[0] == 0): continue
            StdData[i] = iPreNPeriodReportData[-1] + iPreNPeriodYear_1YearReportData[-1] - iPreNPeriodYear_1ReportData[
                -1]
        self._TempData.pop("LastTargetReportDate", None)
        self._TempData.pop("LastTargetReportInd", None)
        return StdData
...
def _saveRawDataWithReportANN(ft, report_ann_file, raw_data, factor_names, raw_data_dir, pid_ids, file_name, pid_lock):
    ...
    if isANNReport:
        ...
        if not os.path.isfile(ANN_ReportFilePath + (
        "." + ft._ANN_ReportFileSuffix if ft._ANN_ReportFileSuffix else "")):  # 没有报告期-公告日期数据, 提取该数据
            with shelve.open(ANN_ReportFilePath) as ANN_ReportFile:
                pass
...
class _AnalystConsensusTable(_DBTable):
    ...
    def __QS_saveRawData__(self, raw_data, factor_names, raw_data_dir, pid_ids, file_name, pid_lock, **kwargs):
        return _saveRawDataWithReportANN(self, self._ANN_ReportFileName, raw_data, factor_names, raw_data_dir, pid_ids,
                                         file_name, pid_lock)
    ...
    def __QS_genGroupInfo__(self, factors, operation_mode):
        ...
        for iFactor in factors:
            ...
            if iPeriod not in PeriodGroup:
                ...
                PeriodGroup[iPeriod]["StartDT"] = min(operation_mode._FactorStartDT[iFactor.Name],
                                                      PeriodGroup[iPeriod]["StartDT"])
        ...
        for iPeriod in PeriodGroup:
            ...
            Groups.append((self, PeriodGroup[iPeriod]["FactorNames"], list(PeriodGroup[iPeriod]["RawFactorNames"]),
                           operation_mode.DTRuler[StartInd:EndInd + 1], PeriodGroup[iPeriod]["args"]))
        return Groups
    ...
    def __QS_calcData__(self, raw_data, factor_names, ids, dts, args={}):
        ...
        if CalcType == "Fwd12M":
            ...
            if (ANNReportPath is not None) and os.path.isfile(
                    ANNReportPath + ("." + self._ANN_ReportFileSuffix if self._ANN_ReportFileSuffix else "")):
...
class _AnalystEstDetailTable(_DBTable):
    ...
    def __QS_prepareRawData__(self, factor_names, ids, dts, args={}):
        ...
        AllFields = list(
            set(factor_names + args.get("附加字段", self.AdditionalFields) + args.get("去重字段", self.Deduplication)))
        DateField = self._DBTableName + "." + self._FactorInfo.loc[self._DateField, "DBFieldName"]
        ReportDateField = self._DBTableName + "." + self._FactorInfo.loc[self._ReportDateField, "DBFieldName"]
...
class JYDB(QSSQLObject, FactorDB):
    ...
    def __init__(self, sys_args={}, config_file=None, **kwargs):
        super().__init__(sys_args=sys_args, config_file=(
            __QS_ConfigPath__ + os.sep + "JYDBConfig.json" if config_file is None else config_file), **kwargs)
        ...
        if not os.path.isfile(self.DBInfoFile):
            ...
            self._TableInfo, self._FactorInfo, self._ExchangeInfo = _updateInfo(self._InfoFilePath,
                                                                                self._InfoResourcePath,
                                                                                self._QS_Logger)  # 数据库表信息, 数据库字段信息
        else:
            self._InfoResourcePath = self.DBInfoFile
            self._TableInfo, self._FactorInfo, self._ExchangeInfo = _updateInfo(self._InfoFilePath,
                                                                                self._InfoResourcePath, self._QS_Logger,
                                                                                out_info=True)  # 数据库表信息, 数据库字段信息
        self.Name = "JYDB"
        return
    ...
    def _getAllAStock(self, date, is_current=True):
        ...
        return [iRslt[0] for iRslt in
                self.fetchall(SQLStr.format(Prefix=self.TablePrefix, Date=date.strftime("%Y-%m-%d")))]
    ...
    def getStockID(self, index_id="全体A股", date=None, is_current=True):  # TODO
        ...
        for iTableName in self._TableInfo[
            (self._TableInfo["TableClass"] == "ConstituentTable") & self._TableInfo.index.str.contains("A股")].index:
    ...
    def getFutureID(self, future_code="IF", exchange="CFFEX", date=None, is_current=True, **kwargs):  # TODO
        ...
        if future_code:
            if isinstance(future_code, str):
                SQLStr += "WHERE fs_info_sccode='" + future_code + "' "
            else:
                SQLStr += "WHERE fs_info_sccode IN ('" + "', '".join(future_code) + "') "
        else:
            SQLStr += "WHERE fs_info_sccode IS NOT NULL "
        ...
        return [iRslt[0] for iRslt in self.fetchall(
            SQLStr.format(Prefix=self.TablePrefix, Date=date.strftime("%Y%m%d"), FutureCode=future_code))]
    ...
    def getOptionID(self, option_code="510050", date=None, is_current=True, **kwargs):
        ...
        SQLStr += "ORDER BY TradingCode"
        return [iRslt[0] for iRslt in self.fetchall(
            SQLStr.format(Prefix=self.TablePrefix, Date=date.strftime("%Y-%m-%d"), OptionCode=option_code))]
