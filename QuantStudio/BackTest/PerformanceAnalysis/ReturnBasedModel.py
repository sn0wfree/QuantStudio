...
class ReturnBasedStyleModel(BaseModule):
    ...
    def __QS_move__(self, idt, **kwargs):
        ...
        self._iDT = idt
        TargetNAV = self._TargetTable.readData(dts=[idt], ids=self._Output["目标ID"], factor_names=[self.TargetNAV]).iloc[
                    0, :, :].values
        self._Output["目标净值"] = np.r_[self._Output["目标净值"], TargetNAV]
        StyleNAV = self._StyleTable.readData(dts=[idt], ids=self._Output["风格ID"], factor_names=[self.StyleNAV]).iloc[0,
                   :, :].values
        ...
        for i, iID in enumerate(self._Output["目标ID"]):
            ...
            try:
                iBeta = regressByCVX(Y[:, i], X, weight=None,
                                     constraints={"Box": {"ub": np.ones((nStyleID,)), "lb": np.zeros((nStyleID,))},
            ...
            if iBeta is None:
                ...
                Rsquared[i] = 1 - np.nansum((Y[:, i][iMask] - np.dot(X[iMask], iBeta)) ** 2) / np.nansum(
    ...
    def __QS_end__(self):
        ...
        self._Output["目标净值"] = pd.DataFrame(self._Output["目标净值"], index=self._Model.DateTimeSeries,
                                            columns=self.TargetIDs)
        self._Output["风格指数净值"] = pd.DataFrame(self._Output["风格指数净值"], index=self._Model.DateTimeSeries,
